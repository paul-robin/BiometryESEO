...
def get_data_label(path,ratio = 0.2):
    """
    Given path returns train and test images and label associated with it
    """
    _data = get_data(path)
    ...
    for keys, vals in _test.items():
        test_image += [[cv.imread(os.path.join(path,files))/255. \
                       for files in vals]]
        test_labels += [keys]  
    return np.array(train_image), np.array(train_labels), np.array(test_image), np.array(np.array(test_labels)),_test
